...
const multipleTypePokemon = pokémon.filter(p => p.types.length > 1);
//console.log(multipleTypePokemon);
const pokemonNames = pokémon.map(p => p.name);
//console.log(pokemonNames);
const pokemonNamesIds = pokémon
    .filter(p => p.id > 99)
    .map(p => p.name);
//console.log(pokemonNamesIds);
const poisonPokemon = pokémon
    .filter(p => p.types == "poison")
    .map(p => p.name);
//console.log(poisonPokemon);
const secondTypeFlyingPokemon = pokémon
    .filter(p => p.types[1] === "flying")
    .map(p => p.types[0]);
//console.log(secondTypeFlyingPokemon);
const normalPokemonCount = pokémon
    .filter(p => p.types == "normal")
    .length;
